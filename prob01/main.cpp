...
int main()
{
  ...
  char my_char1 = 'J';
  short my_short1 = 74;
  std::cout << "The ASCII value for " << static_cast<short>(my_char1) << " is " << static_cast<char>(my_short1) << std::endl;
  ...
  char my_char3 = 'z';
  my_char3 = my_char3 + 1; //ASCII number for the character 'z' is 122.
                           //122 is stored in the memory to represent the ASCII character.
                           //Adding 1 to 122 results in 123. In ASCII 123 is the character '{'
  ...
  std::cout << ", after adding 1: " << std::dec << my_int2
    << std::hex << " (0x" << my_int2 << ")\n";
 /* explanation:
    my_short2 displayed a negative number because it overflowed into the next lowest
    value for the datatype for short which is -32768. The maximum value for short is 32767.
    my_int2's datatype is int which allows for larger numbers, thus it diplayed
    the next incrementally large number.
 */
   std::cout << std::endl; // blank line to separate output from the different exercises
...
std::cout << ", after adding 1: " << std::dec << my_int3
  << std::hex << " (0x" << my_int3 << ")\n";
/* explanation:
   unsigned datatypes do not have negative numbers. When 1 was added to 65535
   it overflowed to the next lowest value for the unsigned short which is 0.
   my_int3 can store larger numberical numbers than short, therefore the value
   was increased to the next larger value and did not over or underflow.
*/
